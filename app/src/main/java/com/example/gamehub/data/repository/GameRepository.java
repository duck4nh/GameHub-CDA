...
/**
 * Lớp trung gian truy cập dữ liệu dùng chung cho ứng dụng.
 *
 * Repository che giấu chi tiết Room, import asset, Firebase và WorkManager để
 * Activity/Fragment/ViewModel chỉ gọi các hàm nghiệp vụ mức cao.
 */
public class GameRepository {
    ...
    /**
     * Đảm bảo câu hỏi Quiz và level Memory đã tồn tại trong Room trước khi chơi.
     * Câu hỏi Quiz được import từ asset DB, còn level Memory được sinh bởi
     * DatabaseSeeder.
     */
    public synchronized void ensureLocalDataReady() throws IOException {
        ...
        // Câu hỏi Quiz nằm trong SQLite asset, sau đó được copy vào bảng Room
        // để các query gameplay có thể lọc và lấy ngẫu nhiên.
    ...
    /**
     * Trả về danh sách câu hỏi Quiz đã lọc cho một ván mới.
     *
     * ViewModel truyền lựa chọn setup của người chơi vào đây. Hàm chọn query DAO
     * phù hợp nhất để Room lọc category/difficulty trước khi RANDOM().
     */
    ...
    /**
     * Trả về toàn bộ level Memory kèm trạng thái mở khóa và best time.
     */
    ...
    /**
     * Cập nhật best time và trạng thái mở khóa sau khi người chơi thắng Memory.
     *
     * Cách xử lý local-first giúp level tiếp theo mở ngay trên máy, còn lịch sử
     * có thể đồng bộ Firebase sau.
     */
    ...
    /**
     * Lưu một ván chơi vào Local_History và đặt lịch đồng bộ nếu bản ghi chưa
     * được upload.
     *
     * Quiz và Memory đều gọi hàm này khi chuyển sang màn kết quả.
     */
    ...
    /**
     * Tìm record local tốt nhất của một game.
     *
     * Hàm có chuẩn hóa tên để các nhãn cũ như "Đố vui" hoặc "Ghi nhớ" vẫn khớp
     * với khóa hiện tại là quiz/memory.
     */
    @Nullable
    ...
    /**
     * Bắt đầu đồng bộ ngay trên IO executor của repository. Nếu có callback,
     * kết quả được trả về main thread để Activity/ViewModel cập nhật UI.
     */
    ...
    /**
     * Đưa job đồng bộ vào WorkManager khi còn bản ghi chưa sync.
     *
     * ExistingWorkPolicy.KEEP tránh tạo nhiều job trùng khi Quiz hoặc Memory lưu
     * nhiều ván trong thời gian ngắn.
     */
    ...
    /**
     * Đẩy lần lượt các bản ghi Local_History đang chờ lên Firebase và trả về
     * thông báo tóm tắt để UI hiển thị.
     */
    private HistorySyncResult syncPendingHistoryBlocking() {
        ...
        for (LocalHistory history : pendingItems) {
            ...
            if (syncResult.success) {
                // Chỉ đánh dấu synced sau khi transaction Firestore xác nhận đã
                // ghi Game_Records và cập nhật Users thành công.
    ...
    private long readDateMillis(DocumentSnapshot document, String field) {
        ...
        // Tương thích dữ liệu cũ: một số bản ghi có thể lưu Unix seconds thay vì milliseconds.
